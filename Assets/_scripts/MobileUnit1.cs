...
public class MobileUnit1 : MonoBehaviour
{
    ...
    [HideInInspector]
    public Factory1 _factory; //used to tell this factory when it has successflly made another factory
    ...
    public void StartConfigure(GameObject go)
    {
        ...
        foreach (GameObject key in keys)
        {
            ...
            go.GetComponentInParent<MobileUnit1>().enabled = false;
            go.GetComponent<CollisionDetection1>().enabled = false;
            break;
        }
    }
    ...
    private void Update()
    {
        if (_reachedTarget)
        {
            ...
            if (allConfig) //if each agent is in position
            {
                ...
                Manager1.Instance.InstantiateFactory(Target);
